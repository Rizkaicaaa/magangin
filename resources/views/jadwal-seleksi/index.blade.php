    ...
    <div id="content-container">
        ...
        <div id="data-state" class="{{ count($jadwals) > 0 ? '' : 'hidden' }}">
            <form action="{{ route('jadwal-seleksi.index') }}" method="GET" class="mb-4 flex">
                ...
                <input type="text" name="search" value="{{ request('search') }}"placeholder="Cari berdasarkan nama atau tempat"
                ...
                <tbody>
                    ...
                    <tr class="border-t">
                        ...
                        <td class="py-3 px-4 flex gap-2">
                            {{-- Tombol Detail --}}
                            <a href="{{ route('jadwal-seleksi.show', $jadwal->id) }}" class="text-green-600 hover:text-green-800" title="Detail">
                                <!-- Icon Eye -->
                            ...
                            {{-- Tombol Edit --}}
                            <a href="{{ route('jadwal-seleksi.edit', $jadwal->id) }}" class="text-blue-600 hover:text-blue-800" title="Edit">
                                <!-- Icon Pencil -->
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            {{-- Tombol Hapus --}}
                            <form action="{{ route('jadwal-seleksi.destroy', $jadwal->id) }}" method="POST" class="inline delete-form">
                                @csrf
                                @method('DELETE')
